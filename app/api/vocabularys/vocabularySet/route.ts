...
export const GET = async (req: NextRequest) => {
  ...
  const vocabularyIdsWithQuizzes = [...new Set(quizzes.map((q) => q.vocabularyId))];

  // Nếu không có vocabulary nào có quiz, trả về mảng rỗng
  if (vocabularyIdsWithQuizzes.length === 0) {
    return NextResponse.json([]);
  }

  // Lấy danh sách vocabularys có quiz
  const data = await db.query.vocabularys.findMany({
    where: inArray(vocabularys.id, vocabularyIdsWithQuizzes),
    with: {
